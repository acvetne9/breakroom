-- Pay filtering at index speed, and word-boundary name scoring.
--
-- role_hourly_rate() ran for every role of every candidate on each query,
-- which pushed pay searches past the 3 s statement timeout. Store the rate on
-- the row instead (generated from salary + pay_period) and index it.

alter table public.business_roles
  add column if not exists hourly_rate numeric
  generated always as (public.role_hourly_rate(salary, pay_period)) stored;

create index if not exists idx_business_roles_business_hourly
  on public.business_roles (business_id, hourly_rate);

create index if not exists idx_business_roles_hourly
  on public.business_roles (hourly_rate)
...
as $$
  with params as (
    select
      ...
      (min_hourly is not null or max_hourly is not null) as has_pay
  ),
  text_hits as (
  ...
  -- Businesses with at least one role in the pay range (index-backed).
  pay_hits as (
    select distinct br.business_id as id
    from public.business_roles br, params p
    where p.has_pay
      and br.hourly_rate between coalesce(min_hourly, 0) and coalesce(max_hourly, 1e9)
  ),
  candidates as (
    select b.id, b.name, b.lat, b.lng, b.business_type, b.address, b.website, b.atmosphere, b.name_norm
    from public.businesses b, params p
    where (cardinality(p.terms) > 0 or p.poly is not null or p.has_box or p.has_pay)
      and (cardinality(p.terms) = 0 or b.id in (select id from text_hits) or b.id in (select id from role_hits))
      and (not p.has_pay or b.id in (select id from pay_hits))
  ...
  select
    ...
    (
      ...
      + case when s.nterms > 0 and s.name_word_hits = s.nterms then 300 else 0 end
      + s.name_word_hits * 50
    ...
    array_remove(array[
      ...
      case when p.has_pay then 'pay' end
