-- Force the term / role / pay scans to run once per query. Without MATERIALIZED
-- the planner inlined them as nested semi-joins and re-ran the trigram scans
-- per candidate, which timed out for neighborhood + pay + term searches.
-- Also add a bounding-box test before ST_Contains so the gist index applies.
...
as $$
  ...
  candidates as (
    ...
    where (cardinality(p.terms) > 0 or p.poly is not null or p.has_box or p.has_pay)
      and (cardinality(p.terms) = 0 or exists (select 1 from matched m where m.id = b.id))
      and (not p.has_pay or exists (select 1 from pay_hits ph where ph.id = b.id))
      and (not p.has_box or (b.lat between min_lat and max_lat and b.lng between min_lng and max_lng))
      and (p.poly is null or (b.geom && p.poly and ST_Contains(p.poly, b.geom)))
  ),
  scored as (
    select
      c.*,
