-- Fix the ambiguous column reference in the search function
...
BEGIN
  ...
  WHERE 
    ...
    (query_terms IS NULL OR (
      ...
      NOT EXISTS (
        SELECT 1 FROM unnest(query_terms) AS query_term
        WHERE NOT (
          lower(b.name) % query_term OR 
          lower(b.name) ILIKE '%' || query_term || '%' OR
          (b.business_type IS NOT NULL AND (
            lower(b.business_type) % query_term OR 
            lower(b.business_type) ILIKE '%' || query_term || '%'
          )) OR
          (br.role IS NOT NULL AND (
            lower(br.role) % query_term OR 
            lower(br.role) ILIKE '%' || query_term || '%'
          ))
        )
      )
    ))
