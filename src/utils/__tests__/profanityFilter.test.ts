...
describe("Profanity Filter", () => {
  describe("isProfane", () => {
    ...
    test("should handle empty or invalid input", () => {
      ...
      expect(isProfane("   ")).toBe(false);
      expect(isProfane(null as unknown as string)).toBe(false);
      expect(isProfane(undefined as unknown as string)).toBe(false);
    });
  ...
  describe("validateContent", () => {
    test("should return error message for profane content", () => {
      expect(validateContent("This is shit")).toBe("Content blocked: inappropriate language detected");
    });

    test("should return null for clean content", () => {
      expect(validateContent("This is a great place")).toBe(null);
    });
  });
});
