...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const hasCurrentJobChanged = () => {
    return currentJob.salary !== initialCurrentJob.salary || currentJob.role !== initialCurrentJob.role || currentJob.location !== initialCurrentJob.location || currentTimePeriod !== initialTimePeriod;
  };
  ...
  useEffect(() => {
    return () => {
      ...
      const hasCurrentJobChangedFromRefs = () => {
        return currentJobRef.current.salary !== initialCurrentJob.salary || currentJobRef.current.role !== initialCurrentJob.role || currentJobRef.current.location !== initialCurrentJob.location || currentTimePeriodRef.current !== initialTimePeriod;
      };
      ...
      if (currentJobChangedRef.current && hasCurrentJobChangedFromRefs() && isCurrentJobCompleteFromRefs()) {
        ...
        onJobUpdate({
          ...
          // Use full location if available
  ...
  const updatePastJob = (id: string, field: keyof Omit<PastJob, 'id'>, value: string) => {
    ...
    const updatedJobs = pastJobs.map(job => job.id === id ? {
      ...job,
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-3">
              <input type="text" inputMode="numeric" value={currentJob.salary} onChange={e => handleSalaryChange(e.target.value)} onBlur={handleCurrentSalaryBlur} className="app-input flex-1" placeholder="$14" />
            ...
            <div className="flex items-center space-x-3">
              <div className="flex-1">
                <BusinessSearchDropdown value={currentJob.location} onChange={handleCurrentJobLocationChange} onBlur={handleCurrentJobLocationBlur} className="app-input w-full" placeholder="Search businesses..." salary={currentJob.salary} role={currentJob.role} timePeriod={currentTimePeriod} />
              </div>
              <div className="w-6"></div>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {pastJobs.map(job => <div key={job.id} className="space-y-3">
                ...
                <div className="flex items-center space-x-3">
                  <BusinessSearchDropdown value={job.location} onChange={value => updatePastJob(job.id, 'location', value)} onBlur={() => handlePastJobBlur(job.id, 'location', job.location)} className="app-input flex-1" placeholder="Search businesses..." salary={job.salary} role={job.role} timePeriod={pastJobTimePeriods[job.id]} />
