...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const [currentJobFullLocation, setCurrentJobFullLocation] = useState<string>(initialData.fullLocation || initialData.location);
  ...
  const isPastJobComplete = (job: PastJob, timePeriod: string) => {
    return job.salary && job.role && job.location && timePeriod && pastJobBusinessSelected[job.id] && (!pastJobShowAddressInputs[job.id] || isValidAddress(pastJobAddresses[job.id]));
  };
  const isCurrentJobComplete = () => {
    return currentJob.salary && currentJob.role && currentJob.location && currentTimePeriod && currentJobBusinessSelected && (!currentJobShowAddressInput || isValidAddress(currentJobAddress));
  };
  useEffect(() => {
    return () => {
      if (hasCreatedPostsRef.current || !onJobUpdate) return;
      hasCreatedPostsRef.current = true;
      const hasCurrentJobChangedFromRefs = () => currentJobRef.current.salary !== initialCurrentJob.salary || currentJobRef.current.role !== initialCurrentJob.role || currentJobRef.current.location !== initialCurrentJob.location || currentTimePeriodRef.current !== initialTimePeriod;
      const isCurrentJobCompleteFromRefs = () => currentJobRef.current.salary && currentJobRef.current.role && currentJobRef.current.location && currentTimePeriodRef.current;
  ...
  return <div className="w-full h-full flex items-center justify-center bg-transparent">
      <div ref={scrollContainerRef} className="app-card p-6 overflow-y-auto w-full h-full">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {/* Business Location */}
            <div>
              <BusinessSearchDropdown value={currentJobBusinessInput} onChange={handleCurrentJobBusinessInputChange} onSelect={handleCurrentJobBusinessSelect} onBlur={handleCurrentJobBusinessBlur} className={`app-input w-full ${currentJobShowAddressInput && !currentJobBusinessSelected ? "border-red-500" : ""}`} placeholder="Where do you work?..." salary={currentJob.salary} role={currentJob.role} timePeriod={currentTimePeriod} />
              
              {/* Address input for current job */}
              {currentJobShowAddressInput && !currentJobBusinessSelected && <div className="mt-2 space-y-2">
                  <p className="text-red-500 text-xs">Business not found. Please enter the address:</p>
                  <input type="text" placeholder="Enter business address (e.g., 123 Main St, City, State)..." className={`app-input w-full ${currentJobAddressError ? 'border-red-500 border-2' : ''}`} value={currentJobAddress} onChange={e => handleCurrentJobAddressChange(e.target.value)} onBlur={handleCurrentJobAddressBlur} />
            ...
            {/* Role */}
            <JobSearchDropdown value={currentJob.role} onChange={handleCurrentJobRoleChange} onBlur={handleCurrentJobRoleBlur} placeholder="Search or select a job role..." className="app-input w-full" />
            ...
            <div className="flex items-center space-x-3">
              ...
              <select value={currentTimePeriod} onChange={e => handleCurrentTimePeriodChange(e.target.value)} className="px-4 py-3 bg-white text-sm" style={{
        ...
        <div>
          ...
          <div className="space-y-4">
            {pastJobs.map(job => <div key={job.id} className="space-y-3 w-full">
                {/* Business Location */}
                <div>
                  <BusinessSearchDropdown value={pastJobBusinessInputs[job.id] || ''} onChange={value => handlePastJobBusinessInputChange(job.id, value)} onSelect={business => handlePastJobBusinessSelect(job.id, business)} onBlur={() => handlePastJobBusinessBlur(job.id)} className={`app-input w-full ${pastJobShowAddressInputs[job.id] && !pastJobBusinessSelected[job.id] ? "border-red-500" : ""}`} placeholder="Where did you work?..." salary={job.salary} role={job.role} timePeriod={pastJobTimePeriods[job.id]} />
                  
                  {/* Address input for past job */}
                  {pastJobShowAddressInputs[job.id] && !pastJobBusinessSelected[job.id] && <div className="mt-2 space-y-2">
                      <p className="text-red-500 text-xs">Business not found. Please enter the address:</p>
                      <input type="text" placeholder="Enter business address (e.g., 123 Main St, City, State)..." className={`app-input w-full ${pastJobAddressErrors[job.id] ? 'border-red-500 border-2' : ''}`} value={pastJobAddresses[job.id] || ''} onChange={e => handlePastJobAddressChange(job.id, e.target.value)} onBlur={() => handlePastJobAddressBlur(job.id)} />
                      {pastJobAddressErrors[job.id] && <p className="text-red-500 text-sm px-1">{pastJobAddressErrors[job.id]}</p>}
                      <p className="text-gray-500 text-xs px-1">
                        Please include street number, street name, and street type (e.g., St, Ave, Rd)
                      </p>
                    </div>}
                ...
                {/* Role */}
                <JobSearchDropdown value={job.role} onChange={value => updatePastJob(job.id, 'role', value)} onBlur={() => handlePastJobRoleBlur(job.id, job.role)} placeholder="Search or select a job role..." className="app-input w-full" />
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <select value={pastJobTimePeriods[job.id] || 'HR'} onChange={e => updatePastJobTimePeriod(job.id, e.target.value)} className="px-4 py-3 bg-white text-sm" style={{
                border: '2px solid hsl(var(--app-gray-light))',
                borderRadius: '0.5rem',
                height: '48px',
                fontSize: '16px'
        ...
        <div className="mt-8">
          ...
          {isStoriesExpanded && <div className="mt-4 space-y-2">
              {userPosts.length === 0 ? <p className="text-app-gray-medium text-sm">No stories or comments yet. Share your workplace experiences!</p> : <>
                  ...
                  {userPosts.length >= 5 && <button onClick={onStoriesClick} className="w-full mt-3 px-4 py-2 bg-app-yellow text-app-black rounded hover:bg-app-yellow/90 transition-colors">
        ...
        <div className="mt-8 flex justify-start relative">
          <button onClick={handleHelpButtonClick} className="w-6 h-6 bg-app-gray-light rounded-full flex items-center justify-center hover:bg-app-gray-medium transition-colors text-app-black font-bold text-sm">
            ?
          </button>
        </div>
